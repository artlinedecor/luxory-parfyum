...
export default function CashflowPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Haqiqatan ham ushbu tranzaksiyani o'chirmoqchimisiz?")) return;
              ...
              {txWithBalance.map((tx) => (
                <tr key={tx.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                  ...
                  <td className="px-6 py-3 text-right">
                    <button onClick={() => handleDelete(tx.id)} className="text-red-400 hover:text-red-300 transition-colors" title="O'chirish">
